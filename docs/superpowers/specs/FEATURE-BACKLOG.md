...
**Resolved (2026-06-11): AI Assist removed entirely** (PR #96, ships in 0.22.0).
Art communities push back on the mere presence of an AI option, a positioning
liability in this audience regardless of feature quality. Removing it RAISES the
priority of **F**: the non-AI, no-key instant generator is now the only quick-fill
path, since there is no longer an AI "Surprise Me" to fall back on.
...
input, a "roll random" hex, and harmony derivation (rotate around one anchor),
but no **non-AI, instant, multi-base** generator. F is press-once → a cohesive
set of bases (not rotations of one), each ramped, no key required.
